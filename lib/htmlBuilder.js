      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            ...
            html[i] = '<script src="' + name.replace(path.basename(name), path.basename(file.path)) + (file.revHash? '?v='+file.revHash : '')+'"></script>';
          resolve();
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          html[i] = '<link rel="stylesheet" href="' + name.replace(path.basename(name), path.basename(file.path))  + (file.revHash ? '?v=' + file.revHash : '') + '"'
